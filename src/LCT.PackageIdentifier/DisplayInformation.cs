...
namespace LCT.PackageIdentifier
{
            ...
            var includeMappings = new Dictionary<string, Func<IEnumerable<string>>>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        { "ALPINE", () => appSettings?.Alpine?.Include }
            ...
            {
                LogHandling.BasicErrorHandelingForLog("Identified invalid projecttype", "DisplayIncludeFiles()", $"Unable to retrieve exclude files because an invalid project type was provided: {appSettings.ProjectType}", "Provide Valid project type in configuration.");
            ...
            var excludeMappings = new Dictionary<string, Func<IEnumerable<string>>>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        { "ALPINE", () => appSettings?.Alpine?.Exclude }
            ...
            {
                LogHandling.BasicErrorHandelingForLog("Identified invalid projecttype", "DisplayExcludeFiles()", $"Unable to retrieve exclude files because an invalid project type was provided: {appSettings.ProjectType}", "Provide Valid project type in configuration.");
                Logger.Error($"Invalid ProjectType - {appSettings.ProjectType}");
            }
        ...
        { "ALPINE", () => appSettings?.Alpine?.Artifactory.InternalRepos }
            ...
            {
                LogHandling.BasicErrorHandelingForLog("Identified invalid projecttype", "GetInternalRepolist()", $"Unable to retrieve exclude files because an invalid project type was provided: {appSettings.ProjectType}", "Provide Valid project type in configuration.");
                Logger.Error($"Invalid ProjectType - {appSettings.ProjectType}");
            }
