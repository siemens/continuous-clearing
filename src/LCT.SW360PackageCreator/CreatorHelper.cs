...
namespace LCT.SW360PackageCreator
{
    ...
    public class CreatorHelper : ICreatorHelper
    {
        ...
        private static async Task<string> GetAttachmentUrlList(ComparisonBomData component, string localPathforDownload)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetAttachmentUrlList", $"MethodName:GetAttachmentUrlList(), Release Name: {component.Name}@{component.Version}, SourceUrl: {component.SourceUrl}", ex, "A network error occurred while trying to download the attachment.");
            }
            catch (UriFormatException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetAttachmentUrlList", $"MethodName:GetAttachmentUrlList(), Release Name: {component.Name}@{component.Version}, SourceUrl: {component.SourceUrl}", ex, "The provided URL is not in a valid format.");
            }
        ...
        public async Task<List<ComparisonBomData>> SetContentsForComparisonBOM(List<Components> lstComponentForBOM, ISW360Service sw360Service)
        {
            ...
            LogHandling.LogAvailableComponentList(componentsAvailableInSw360);
            ...
            LogHandling.LogComparitionComponentListData(comparisonBomData);
            ...
            ISW360Service sw360Service, Bom bom)
        {
            ...
            foreach (ComparisonBomData comBom in updatedCompareBomData)
            {
                ...
                {
                    LogHandling.HttpErrorHandelingForLog("GetUpdatedComponentsDetails", $"MethodName:GetUpdatedComponentsDetails(), ComponentName:{comBom.Name}, Version:{comBom.Version}", ex, "An error occurred while serializing or deserializing JSON data for the component.");
                }
            }
            return bom;
        }
        ...
        private static string GetDownloadPathForComponetType(ComparisonBomData component)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetDownloadPathForComponetType", $"MethodName:GetDownloadPathForComponetType(), ComponentName:{component.Name}, ReleaseExternalId:{component.ReleaseExternalId}", ex, "An I/O error occurred while determining the download path for the component.");
                Logger.Error($"GetDownloadPathForComponetType() ", ex);
            }
            catch (UnauthorizedAccessException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetDownloadPathForComponetType", $"MethodName:GetDownloadPathForComponetType(), ComponentName:{component.Name}, ReleaseExternalId:{component.ReleaseExternalId}", ex, "Unauthorized access occurred while determining the download path for the component.");
                Logger.Error($"GetDownloadPathForComponetType() ", ex);
            }
