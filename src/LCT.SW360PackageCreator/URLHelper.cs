...
namespace LCT.SW360PackageCreator
{
    ...
    public class UrlHelper : IUrlHelper, IDisposable
    {
        ...
        public async Task<Components> GetSourceUrlForAlpinePackage(string componentName, string componenVersion, string bomRef)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceUrlForAlpinePackage", $"MethodName:GetSourceUrlForAlpinePackage(), ComponentName: {componentName}, Version: {componenVersion}", ex, "An I/O error occurred while processing the Alpine package source URL.");
        ...
        private static Task<AlpinePackage> GetAlpineSourceUrl(string name, string version, string localPathforSourceRepo)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetAlpineSourceUrl", $"MethodName:GetAlpineSourceUrl(), PackageName: {name}, Version: {version}, LocalPath: {localPathforSourceRepo}", ex, "An I/O error occurred while trying to retrieve the Alpine source URL.");
                Logger.Error($"GetAlpineSourceUrl() ", ex);
            }
        ...
        public static string GetSourceFromAPKBUILD(string localPathforSourceRepo, string name)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceFromAPKBUILD", $"MethodName:GetSourceFromAPKBUILD(), PackageName: {name}, LocalPath: {localPathforSourceRepo}", ex, "An I/O error occurred while trying to read the APKBUILD file.");
                Logger.Error($"GetSourceFromAPKBUILD() ", ex);
            }
            catch (UnauthorizedAccessException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceFromAPKBUILD", $"MethodName:GetSourceFromAPKBUILD(), PackageName: {name}, LocalPath: {localPathforSourceRepo}", ex, "Unauthorized access occurred while trying to read the APKBUILD file.");
                Logger.Error($"GetSourceFromAPKBUILD() ", ex);
            }
        ...
        public static string GetSourceUrlForAlpine(string pkgFilePath, string sourceData)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceUrlForAlpine", $"MethodName:GetSourceUrlForAlpine(), PackageFilePath: {pkgFilePath}", ex, "An I/O error occurred while trying to read the package file.");
                Logger.Error($"GetDownloadPathForAlpineRepo() ", ex);
            }
            catch (UnauthorizedAccessException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceUrlForAlpine", $"MethodName:GetSourceUrlForAlpine(), PackageFilePath: {pkgFilePath}", ex, "Unauthorized access occurred while trying to read the package file.");
                Logger.Error($"GetDownloadPathForAlpineRepo() ", ex);
            }
        ...
        public static string GetDownloadPathForAlpineRepo()
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetDownloadPathForAlpineRepo", "MethodName:GetDownloadPathForAlpineRepo()", ex, "An I/O error occurred while trying to create or access the directory.");
                Logger.Error($"GetDownloadPathForAlpineRepo() ", ex);
            }
            catch (UnauthorizedAccessException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetDownloadPathForAlpineRepo", "MethodName:GetDownloadPathForAlpineRepo()", ex, "Unauthorized access occurred while trying to create or access the directory.");
                Logger.Error($"GetDownloadPathForAlpineRepo() ", ex);
            }
        ...
        public async Task<string> GetSourceUrlForConanPackage(string componentName, string componenVersion)
        {
            ...
            using (HttpClient _httpClient = new HttpClient())
            {
                ...
                catch (HttpRequestException ex)
                {
                    ...
                    LogHandling.HttpErrorHandelingForLog("GetSourceUrlForConanPackage", $"MethodName:GetSourceUrlForConanPackage(), ComponentName: {componentName}, Version: {componenVersion}, URL: {downLoadUrl}", ex, "An HTTP request error occurred while trying to fetch the Conan package source URL.");
                }
                catch (YamlException ex)
                {
                    Logger.Warn($"Identification of SRC url failed for {componentName}, " +
                                    $"Exclude if it is an internal component or manually update the SRC url");
                    LogHandling.HttpErrorHandelingForLog("GetSourceUrlForConanPackage", $"MethodName:GetSourceUrlForConanPackage(), ComponentName: {componentName}, Version: {componenVersion}, URL: {downLoadUrl}", ex, "A YAML parsing error occurred while trying to deserialize the Conan package data.");
                }
                catch (ArgumentNullException ex)
                {
                    LogHandling.HttpErrorHandelingForLog("GetSourceUrlForConanPackage", $"MethodName:GetSourceUrlForConanPackage(), ComponentName: {componentName}, Version: {componenVersion}, URL: {downLoadUrl}", ex, "A null argument was encountered while processing the Conan package source URL.");
                }
            }
        ...
        private async Task<string> GetSourceURLFromNuspecFile(string nuspecURL, string componentName)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromNuspecFile", $"MethodName:GetSourceURLFromNuspecFile(), ComponentName: {componentName}, NuspecURL: {nuspecURL}", ex, "Multiple errors occurred while processing the Nuspec file. Please investigate the inner exceptions for more details.");
            }
            catch (HttpRequestException ex)
            {
                Logger.Warn($"Identification of SRC url failed for {componentName}, " +
            $"Exclude if it is an internal component or manually update the SRC url");
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromNuspecFile", $"MethodName:GetSourceURLFromNuspecFile(), ComponentName: {componentName}, NuspecURL: {nuspecURL}", ex, "An HTTP request error occurred while trying to fetch the Nuspec file.");
            }
            return GithubUrl;
        }
        ...
        private async Task<DebianPackage> GetArchiveResponse(DebianPackage packageDetails, string packageType)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetArchiveResponse", $"MethodName:GetArchiveResponse(), PackageName: {packageDetails.Name}, PackageType: {packageType}", ex, "An HTTP request error occurred while trying to fetch the archive response.");
        ...
        private static DebianPackage GetSourceURLFromJsonTextForSourceType(DebianPackage sourceURLDetails)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromJsonTextForSourceType", $"MethodName:GetSourceURLFromJsonTextForSourceType(), JsonText: {sourceURLDetails.JsonText}", ex, "An error occurred while parsing the JSON text.");
            }
            catch (IOException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromJsonTextForSourceType", $"MethodName:GetSourceURLFromJsonTextForSourceType(), JsonText: {sourceURLDetails.JsonText}", ex, "An I/O error occurred while processing the source URL.");
            }
            return sourceURLDetails;
        }
        ...
        private async Task<DebianPackage> GetSourceURLFromJsonTextForBinaryType(DebianPackage sourceURLDetails)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromJsonTextForBinaryType", $"MethodName:GetSourceURLFromJsonTextForBinaryType(), JsonText: {sourceURLDetails.JsonText}", ex, "An error occurred while parsing the JSON text.");
            }
            catch (IOException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromJsonTextForBinaryType", $"MethodName:GetSourceURLFromJsonTextForBinaryType(), JsonText: {sourceURLDetails.JsonText}", ex, "An I/O error occurred while processing the binary type source URL.");
            }
        ...
        private async Task<DebianPackage> RetryToGetSourceURlDetailsAsync(string name, string version)
        {
            ...
            {
                Logger.Warn($"RetryToGetSourceURlDetailsAsync(): Source package not found for ComponentName: {name}, Version: {version}");
                Logger.Debug($"RetryToGetSourceURlDetailsAsync(): Retry failed for ComponentName: {name}, Version: {version}");
            }
        ...
        private async Task<string> GetResponseFromPyPiOrg(string componentName, string componenVersion)
        {
            ...
            try
            {
                ...
                LogHandling.LogRequestDetails("GetResponseFromPyPiOrg", $"MethodName:GetResponseFromPyPiOrg(), ComponentName: {componentName}, Version: {componenVersion}", httpClient, URL);
                var response = await httpClient.GetStringAsync(URL);
                LogHandling.LogHttpResponseDetailsForStringContent("GetResponseFromPyPiOrg", $"MethodName:GetResponseFromPyPiOrg(), ComponentName: {componentName}, Version: {componenVersion}", response);
                return response.ToString();
            }
            catch (HttpRequestException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetResponseFromPyPiOrg", $"MethodName:GetResponseFromPyPiOrg(), ComponentName: {componentName}, Version: {componenVersion}", ex, "An HTTP request error occurred while trying to fetch the response from PyPi.");
            }
            catch (TaskCanceledException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetResponseFromPyPiOrg", $"MethodName:GetResponseFromPyPiOrg(), ComponentName: {componentName}, Version: {componenVersion}", ex, "The request to PyPi was canceled, possibly due to a timeout.");
            }
            return result;
        }
        ...
        private static string GetSourceURLFromPyPiResponse(string response)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromPyPiResponse", "MethodName:GetSourceURLFromPyPiResponse()", ex, "An error occurred while parsing the JSON response from PyPi.");
            }
            catch (IOException ex)
            {
                LogHandling.HttpErrorHandelingForLog("GetSourceURLFromPyPiResponse", "MethodName:GetSourceURLFromPyPiResponse()", ex, "An I/O error occurred while processing the PyPi response.");
            }
            return SourceURL;
        }
        ...
        public static async Task<string> DownloadFileAsync(Uri uri, string downloadFilePath)
        {
            ...
            {
                LogHandling.HttpErrorHandelingForLog("DownloadFileAsync", $"MethodName:DownloadFileAsync(), FilePath: {downloadFilePath}, URI: {uri}", webex, "A network error occurred while trying to download the file.");
            }
