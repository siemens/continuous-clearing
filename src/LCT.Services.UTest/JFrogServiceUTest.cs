...
namespace LCT.Services.UTest
{
    ...
    internal class JFrogServiceUTest
    {
        ...
        [Test]
        public async Task GetCargoComponentDataByRepo_GetsRepoInfo_Successfully()
        {
            // Arrange
            AqlResult aqlResult = new AqlResult()
            {
                Name = "example-package-1.0.0.crate",
                Path = "@testfolder/-/folder",
                Repo = "crate-repo"
            };
            ...
            Mock<IJfrogAqlApiCommunicationFacade> mockJfrogApiComFacade = new Mock<IJfrogAqlApiCommunicationFacade>();
            mockJfrogApiComFacade
                .Setup(x => x.GetCargoComponentDataByRepo(It.IsAny<string>()))
                .ReturnsAsync(httpResponseMessage);

            // Act
            IJFrogService jFrogService = new JFrogService(mockJfrogApiComFacade.Object);
            IList<AqlResult> actual = await jFrogService.GetCargoComponentDataByRepo("cargo-repo");

            // Assert
            Assert.That(actual.Count, Is.GreaterThan(0));
            Assert.That(actual[0].Name, Is.EqualTo("example-package-1.0.0.crate"));
        }

        [Test]
        public async Task GetCargoComponentDataByRepo_ResultsWith_NoContent()
        {
            // Arrange
            HttpResponseMessage httpResponseMessage = new HttpResponseMessage(HttpStatusCode.NoContent);
        ...
        [Test]
        public async Task GetCargoComponentDataByRepo_ResultsWith_HttpRequestException()
        {
            // Arrange
            Mock<IJfrogAqlApiCommunicationFacade> mockJfrogApiComFacade = new Mock<IJfrogAqlApiCommunicationFacade>();
            mockJfrogApiComFacade
        ...
        [Test]
        public async Task GetCargoComponentDataByRepo_ResultsWith_InvalidOperationException()
        {
            // Arrange
            Mock<IJfrogAqlApiCommunicationFacade> mockJfrogApiComFacade = new Mock<IJfrogAqlApiCommunicationFacade>();
            mockJfrogApiComFacade
        ...
        [Test]
        public async Task GetCargoComponentDataByRepo_ResultsWith_TaskCanceledException()
        {
            // Arrange
            Mock<IJfrogAqlApiCommunicationFacade> mockJfrogApiComFacade = new Mock<IJfrogAqlApiCommunicationFacade>();
            mockJfrogApiComFacade
