...
namespace SW360IntegrationTest.NPM
{
    ...
    public class SITScanWithMultiplePackageLockInputs
    {
        ...
        public void Setup()
        {
            ...
            CCTLocalBomTestFile = Path.GetFullPath(Path.Combine(OutFolder, "..", "..", "src", "SW360IntegrationTest", "SITScanTestFiles", "Npm", "CCTLocalBOMNpmMultiplePackages.json"));
        ...
        public void TestLocalBOMCreation()
        {
                ...
                foreach (var item in expected.Components)
                {
                    ...
                    foreach (var i in actual.Components)
                    {
                        if ((i.Name == item.Name) && (i.Version == item.Version))
                        {
                            ...
                            Assert.AreEqual(item.Version, component.Version, "Test to check version");
                            Assert.AreEqual(item.Purl, component.Purl, "Test to check component externalid");
                            Assert.AreEqual(item.BomRef, component.BomRef, "Test to check release externalid");
                        }
                    }
            ...
            Assert.IsTrue(fileExist, "Test to local BOM file present");
        }
