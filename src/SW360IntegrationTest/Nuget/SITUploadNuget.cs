...
namespace SW360IntegrationTest.Nuget
{
    ...
    public class SITUploadNuget
    {
        private string OutFolder { get; set; }
        private static readonly TestParamNuget testParameters = new TestParamNuget();
        [Test, Order(1)]
        public void TestSITUploadexe()
        {
            OutFolder = Path.GetFullPath(Path.Combine(TestHelper.OutFolder, "..", "..", "TestFiles", "IntegrationTestFiles", "SITUploadTestData", "Nuget"));

            // Test BOM Creator ran with exit code 0
            Assert.AreEqual(0, TestHelper.RunSITUploadExe(new string[]{
        ...
        public void ComponentUpload_IsUnsuccessful_AlreadyPresentInDestination()
        {
            ...
            string comparisonBOMPath = Path.GetFullPath(Path.Combine(OutFolder, "..", "..", "TestFiles", "IntegrationTestFiles", "SITUploadTestData", "Nuget", "Test_Bom.cdx.json"));
