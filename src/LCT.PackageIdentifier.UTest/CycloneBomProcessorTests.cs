...
namespace PackageIdentifier.UTest
{
    public class CycloneBomProcessorTests
    {
        ...
        [Test]
        public void AddComponentHashes_ShouldCalculateHashesForAllComponents()
        {
            // Arrange
            var parser = new CycloneDXBomParser();
            var bom = new Bom
            {
                Components = new List<Component>
                {
            ...
            // Act
            parser.AddComponentHashes(bom);

            // Assert
            Assert.AreEqual(2, bom.Components.Count);
            ...
            foreach (var component in bom.Components)
            {
                ...
                {
                    Assert.IsNotNull(hash.Content);
                }
            }
        }

        [Test]
        public void GetHashForComponent_ShouldReturnValidHash()
        {
            // Arrange
     
            var input = "Component1-1.0.0";

            // Act
            var md5Hash = CycloneDXBomParser.GetHashForComponent(input, MD5.Create());
            var sha1Hash = CycloneDXBomParser.GetHashForComponent(input, SHA1.Create());
            var sha256Hash = CycloneDXBomParser.GetHashForComponent(input, SHA256.Create());
