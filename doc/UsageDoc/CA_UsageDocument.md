...
## Supported Package Managers and Input Files

| Package Manager | Version | Input File | Generated By | Command |
|----------------|---------|------------|--------------|---------|
| NPM | All | package-lock.json | npm install | `npm install` |
| NuGet | All | packages.config / project.assets.json | dotnet restore | `dotnet restore` |
| Maven | All | bom.cdx.json | cyclonedx:makeAggregateBom | `mvn install cyclonedx:makeAggregateBom` |
| Python Poetry | All | poetry.lock | poetry install | `poetry install` |
| **Conan** | **v2.x** | **dep.json** | **conan graph info** | **`conan graph info . -f json > dep.json`** |
| Debian | All | *.cdx.json | syft or manual | `syft <image> -o cyclonedx-json` |
| Alpine | All | *.cdx.json | syft or manual | `syft <image> -o cyclonedx-json` |
